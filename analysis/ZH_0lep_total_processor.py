...
def build_sum_vector(jets, mask, n):
    ...
    Parameters:
        jets: awkward.Array  -- nested jagged array of jets
        mask: awkward.Array  -- boolean mask (same length as jets)
        n: int               -- number of jets to sum (first n jets per event)

    Returns:
        LorentzVector awkward.Array: summed vector per event
    '''
...
class TOTAL_Processor(processor.ProcessorABC):
    def __init__(self, xsec=0.89, lumi=41.5e3, nevts=3000, isMC=False, dataset_name=None):
        self.xsec = xsec
        self.lumi = lumi
        self.nevts = nevts
    ...
    def process(self, events):
        ...
        if self.dataset_name and self.dataset_name.startswith("ZH_ZToAll_HToAATo4B"):
            # Compute on the fly
        ...
        output["dphi_H_MET_merged"].fill(dphi=vec_H_merged.delta_phi(make_vector_met(puppimet_all)),weight=weights_merged_all)
        ...
        output["dphi_untag_MET_merged"].fill(
            dphi=ak.to_numpy(first_untag_merged[valid_mask_merged].delta_phi(puppimet_merged[valid_mask_merged])),
            weight=ak.to_numpy(weights_merged[valid_mask_merged])
        )
